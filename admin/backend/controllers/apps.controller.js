import { getApps, getSchemas, createApp as  _createApp } from '../utils/file-scanner.util.js';
...
export const getAllApps = (req, res) => {
  const apps = getApps(`${process.cwd()}/src/apps`).map((e, i)=> {return {id: i, app: e}});
  res.json({ apps });
};
...
export const createApp = (req, res) => {
  try {
    if (!req.body || !req.body.appName) {
      return res.status(400).json({ error: 'appName is required in request body' });
    }

    const result = _createApp(`${process.cwd()}/src/apps/${req.body.appName}`);
